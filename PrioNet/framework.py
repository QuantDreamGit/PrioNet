...
class Framework:
    def __init__(self, 
                 ...
                 quiet_mode=False, testnet=False):
        # Set the bot name
    ...
    def set_parameters(self, symbol, qty, tp, sl):
        ...
        min_qty = self.bybit.get_precision(symbol)

        # Check if the quantity is higher than the minimum quantity
        if qty < min_qty:
            # Send a message to the Telegram chat
            self.telegram.send_message(f'{self.bot_name} quantity is lower than the minimum quantity for {symbol}.')
            # Return the error message
            return f'Quantity is lower than the minimum quantity for {symbol}.'
